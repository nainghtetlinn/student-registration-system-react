...
const uploadSignature = (
  ...
  const params =
    type == 'Guardian Signature' ? { type, guardianName } : { type }

  return api.patch<ApiResponse<string>>(
    '/student/registrationForm/uploadSignature',
    formData,
    { params },
  )
}
...
export const useUploadStudentSignature = (
  ...
  return useMutation({
    mutationKey: ['form', 'registration', 'upload-signature', 'student'],
...
export const useUploadGuardianSignature = (
  ...
  return useMutation({
    mutationKey: ['form', 'registration', 'upload-signature', 'guardian'],
    mutationFn: async (data) => {
